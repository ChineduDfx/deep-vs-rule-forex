...
def build_model(input_shape):
    """Build and compile a single-layer LSTM classification model.
...
def prepare_sequences(df, feature_names, target_col, seq_len):
    """Convert a flat dataframe into sliding-window sequences for the LSTM.
...
def evaluate(model, X_test, y_test):
    ...
    # Analytic FLOPs: 8*d^2 + 8*d*m + 12*d
    flops = 8 * HIDDEN_DIM ** 2 + 8 * HIDDEN_DIM * INPUT_DIM + 12 * HIDDEN_DIM
